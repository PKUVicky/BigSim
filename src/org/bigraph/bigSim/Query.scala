...
/*class QueryVal extends Query {
	
	override def toString = "<uninitialised queryVal>";
	
	override def check(n : Node) = {
		println("queryVal.check hit invalid node in check()");
		false;		
	}
	
	override def eval(n : Node) = {
		println("queryVal.eval hit invalid node in eval()");
		0;
	}
...
/*class QueryId(id : String) extends QueryVal {
	var name : String = id;
	...
	override def check(n : Node) = {
		if (name == "true") true;
		else if (name == "false") false;
		else {
			println("queryId.check, unknown id " + name + " in property");
			false;
		}
	}
	
	override def eval(n : Node) = {
		println("queryId.eval, invalid integer id in eval()");
		0;
	}
}*/
