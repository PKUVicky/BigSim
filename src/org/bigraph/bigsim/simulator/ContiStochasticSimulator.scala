...
object ContiStochasticSimulator {
...
class ContiStochasticSimulator(b: Bigraph) extends Simulator {
  ...
  def simulate: Unit = {
    ...
    /**
     * Store time and react match
     */
    var workingMap: Map[Double, Set[Match]] = Map()
    var reactNodes: Set[String] = Set()
    ...
    while (systemActivity > 0 || workingMap.size > 0) {

      /**
       * 1. Update reaction if time > workingMap's reaction time
       */
      workingMap.filter(p => p._1 <= time).map(w => {
        ...
        w._2.map(m => {
          ...
          if (ms.size > 0) {
            ...
            if (curb != null && newb != null) {
              ...
              if (newb.root == null)
                newb.root = new Nil();
              newv = new Vertex(newb, curv, m.rule)
              newv.CLK = time.toInt
       ...
       */
      // sample rule
      val ru: Int = ContiStochasticSimulator.InversionSampling(ruleActivities.filter(r => r._2 > 0).map(r => { r._1 -> r._2 / systemActivity }))
    ...
    ContiStochasticSimulator.matchGC;
  }
